                ...
                <div class="p-4">
                    <div class="grid gap-6 mb-2 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="name{{$user->id}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="John Doe" value="{{$user->name}}" disabled />
                        </div>
                        <div>
                            <label for="email{{$user->id}}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                            <input type="email" id="email{{$user->id}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="example@example.com" value="{{$user->email}}" disabled />
                        </div>
                    </div>
                    <div class="text-gray-900 dark:text-gray-100 sm:flex gap-1 flex justify-end">
                        ...
                        <form class="m-0" id="deleteForm{{$user->id}}" action='{{ route("user.delete", $user->id) }}' method="POST">
                            @csrf
                            @method('DELETE')
